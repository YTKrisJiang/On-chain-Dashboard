...
// 配置 OpenAI SDK 使用 OpenRouter
const openai = new OpenAI({
  apiKey: process.env.DEEPSEEK_API_KEY, // 使用 DEEPSEEK_API_KEY 环境变量存储 OpenRouter API 密钥
  baseURL: "https://openrouter.ai/api/v1",
  defaultHeaders: {
    "HTTP-Referer": "https://solana-wallets-monitor.vercel.app", // 你的网站 URL
    "X-Title": "Solana Wallets Monitor" // 你的网站名称
  }
});
...
${promptSuffix}`;
    ...
    // 使用 OpenRouter API 调用
    try {
      const response = await openai.chat.completions.create({
        model: "deepseek/deepseek-r1:free", // 使用 OpenRouter 支持的模型
        messages: [
      ...
      const fallbackResponse = await openai.chat.completions.create({
        model: "deepseek/deepseek-chat:free", // 备用模型
        messages: [
          { role: "system", content: "You are a helpful assistant that analyzes cryptocurrency Twitter data." },
          { role: "user", content: prompt }
        ],
...
export async function sendSumMessage(tokenInfo, replyToMessageId) {
  try {
    ...
    console.error("Error in sendSumMessage:", error);
    return null;
  }
}